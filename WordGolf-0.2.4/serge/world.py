...
class World(common.Loggable, serialize.Serializable, common.EventAware):
    ...
    def findActorsAt(self, x, y):
        ...
        for actor in self.getActors():
            if test.isInside(actor):
                actors.append(actor)
        return actors
        
    def getActors(self):
        """Return all the actors"""
        actors = list(self.unzoned_actors)
        for z in self.zones:
    ...
    def addActor(self, actor):
        ...
        #
        self.log.info('Adding %s to world %s' % (actor.getNiceName(), self.name))
        #
    ...
    def removeActor(self, actor):
        ...
        self.log.info('Removing "%s" actor (%s)' % (actor.tag, actor.getNiceName()))
        #
        # Tell the actor about it
    ...
    def processEvents(self, events):
        """Handle the events"""
        for (name, source), actor in events:
            if actor.active:
                actor.processEvent((name, source))
